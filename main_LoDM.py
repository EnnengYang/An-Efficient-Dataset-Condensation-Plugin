...
def get_loops(ipc):
    ...
    elif ipc == 7: # Only LoRa
        outer_loop, inner_loop = 1, 1
    elif ipc == 10:
        outer_loop, inner_loop = 10, 50
    elif ipc == 70: # Only LoRa
        outer_loop, inner_loop = 10, 50
    elif ipc == 50:
        outer_loop, inner_loop = 50, 10
    elif ipc == 350: # Only LoRa
        outer_loop, inner_loop = 50, 10
    else:
...
def main():
    ...
    args.save_path += args.dataset +'/'+'LoDM_' + str(args.ipc) + "ipc_" + str(args.init) + '/' + str(str_time) + '/'
    ...
    for exp in range(args.num_exp):
        ...
        for it in range(args.Iteration+1):
            ...
            ''' visualize and save '''
            if it % 500 ==0:
                save_name = os.path.join(args.save_path, 'lodm_vis_%s_%s_%s_%dipc_exp%d_iter%d.png' % (args.method, args.dataset, args.model, args.ipc, exp, it))
                image_syn = image_syn_a.matmul(image_syn_b)
