...
def main():
    ...
    args.save_path += args.dataset +'/'+'LoDC_' + str(args.ipc) + "ipc"+ str(args.rank) + "rank" +'/'+ str(str_time)+'/'
    if not os.path.exists(args.save_path):
        os.makedirs(args.save_path)

    logger = create_log_dir(args.save_path, 'log_rank_'+str(args.rank)+'.txt')
    ...
    for exp in range(args.num_exp):
        ...
        ''' training '''
        optimizer_img = torch.optim.SGD([image_syn_a, image_syn_b], lr=args.lr_img, momentum=0.5) # optimizer_img for synthetic data
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool:
                ...
                ''' visualize and save '''
                save_name = os.path.join(args.save_path, 'lodc_vis_%s_%s_%s_%dipc_exp%d_iter%d.png'%(args.method, args.dataset, args.model, args.ipc, exp, it))
                image_syn = image_syn_a.matmul(image_syn_b)
