...
from networks_mtt import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'MNIST':
        ...
        #     transform = None
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR10(data_path, train=True, download=False, transform=transform)  # no augmentation
        ...
        loader_train_dict = {
            c: torch.utils.data.DataLoader(dst_train_dict[c], batch_size=batch_size, shuffle=True, num_workers=0) for c
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True, only_feat=False):
    ...
    if model == 'MLP':
        ...
        net = ConvNet(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size, only_feat=only_feat)
    elif model == 'LeNet':
