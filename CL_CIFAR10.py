...
def main(method_=None, steps_=-1):
    parser = argparse.ArgumentParser(description='Parameter Processing')
    parser.add_argument('--method', type=str, default='LoDM', help='random/DSA/DM/LoDM/')
    ...
    logger = create_log_dir(args.save_path, 'cifar10_log_steps_'+ str(args.steps) + '.txt')
    ...
    results = np.zeros((args.steps, 2*args.num_eval))
    ...
    for seed_cl in range(5):
        ...
        if args.method == 'random':
            ...
            fname = os.path.join(args.cl_data_path, 'cl_DM_CIFAR10_ConvNet_10ipc_%dsteps_seed%d.pt'%(args.steps, seed_cl))
            ...
            fname = os.path.join(args.cl_data_path, 'cl_DM_CIFAR10_ConvNet_80ipc_%dsteps_seed%d.pt' % (args.steps, seed_cl))
            data = torch.load(fname, map_location='cpu')['data']
            images_train_all = [data[step][0] for step in range(args.steps)]
            labels_train_all = [data[step][1] for step in range(args.steps)]
            logger.info('use data: ' + str(fname))
